...
List caisa_acc        (const arma::mat& X,
  ...
  for (iter = 0; iter < maxiter; iter++) {
    ...
    for (j = 0; j < p; j++){
      ...
      updatebetaj(X.col(j), w(j), beta(j), r, L, pi, piold, sigma2, sa2,
    ...
    // UPDATE PI BY MIXSQP
    // ---------------------------------------------------------------------
    pi                    = mixsqp_rcpp(L, piold, 1e-8, 1e-8, 1e-4, 1e-4, 1e-2, 0.5,
                                        1e-4, 1e-8, 1e-8, 100, 20, FALSE);
    pi                    = pi/sum(pi);
