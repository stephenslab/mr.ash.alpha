...
for (i in 1:20) {
  ...
  beta              = double(p)
  nzind             = which(sample(2, p, replace = TRUE, prob = c(0.025,0.975)) == 1)
...
#### Signal = SparseLaplace, PVE = 0.9

```{r sparselaplace0.9, eval = FALSE}
...
for (i in 1:20) {
  ...
  beta[nzind]       = rexp(length(nzind)) * sign(rnorm(length(nzind)))
  sa2               = (sqrt(1.5)^(0:19) - 1)^2
  sigma             = sqrt(20 / 9)
  y                <- X %*% beta + sigma * rnorm(500)
  err.test          = sigma * rnorm(500)
...
### Signal = SparseLaplace, PVE = 0.5

```{r sparselaplace0.5, eval = FALSE}
...
#### Signal = SparseLaplace, PVE = 0.1

```{r sparselaplace0.1, eval = FALSE}
...
for (nums in numlist) {
  ...
  write.table(df, paste(paste("../paperresults/pred6_", nums, sep = ""), ".txt", sep = ""), sep = ",")
  
  df =   data.frame("fit" = rep(c("CAISA","SUSIE","VARBVS","L0Learn","LASSO",
                                  "RIDGE","E-NET","BLASSO","BayesB"), each = 20),
                    "time" = c(get(paste("t", nums, sep = ""))))
  write.table(df, paste(paste("../paperresults/time6_", nums, sep = ""), ".txt", sep = ""), sep = ",")
}
```
...
for (i in 1:length(numlist)) {
  num = numlist[i]
  dat[[i]] = read.table(paste(paste("../paperresults/pred6_", num, sep = ""), ".txt", sep = ""), sep = ",")
  ...
  dat[[i]]$time = read.table(paste(paste("../paperresults/time6_", num, sep = ""), ".txt", sep = ""), sep = ",")$time
  dat[[i]]$rrmse2 = pmin(pmax(dat[[i]]$rrmse, 0),1)
  dat[[i]]$sigma = get(paste("sigma", num, sep = ""))
  dat[[i]]$p = 2000
}
saveRDS(dat, "../paperresults/experiment6.rds")
```

```{r readresults, message = FALSE}
dat = readRDS("../paperresults/experiment6.rds")
for (i in 1:9) {
...
title = ggdraw() + draw_label("EquiCorrGauss + SparseNormal", fontface = 'bold', size = 18)
fig1_temp = plot_grid(p11,p12,p13, nrow = 1)
fig1 = plot_grid(title,fig1_temp, ncol = 1, rel_heights = c(0.06,0.95))
ggsave("../paperfigures/fig61.pdf", fig1, width = 18, height = 6)
```
...
title = ggdraw() + draw_label("EquiCorrGauss + SingleEffect", fontface = 'bold', size = 18)
fig2_temp = plot_grid(p21,p22,p23, nrow = 1)
fig2 = plot_grid(title,fig2_temp, ncol = 1, rel_heights = c(0.06,0.95))
ggsave("../paperfigures/fig62.pdf", fig1, width = 18, height = 6)
```
...
title = ggdraw() + draw_label("EquiCorrGauss + SparseLaplace", fontface = 'bold', size = 18)
fig3_temp = plot_grid(p31,p32,p33, nrow = 1)
fig3 = plot_grid(title,fig3_temp, ncol = 1, rel_heights = c(0.06,0.95))
ggsave("../paperfigures/fig63.pdf", fig1, width = 18, height = 6)
```
