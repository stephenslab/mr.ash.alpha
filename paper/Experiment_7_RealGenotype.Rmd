---
title: "Experiment 7 (RealGenotype)"
...
for (i in 1:20) {
  ...
  fit.mrash        <- mr_ash(X = X, y = y, sa2 = sa2,
                             stepsize = 1, max.iter = 2000,
                             standardize = standardize, beta.init = fit.susie$beta,
                             tol = list(epstol = 1e-12, convtol = 1e-8)))
...
for (nums in numlist) {
  ...
  write.table(df, paste(paste("../paperresults/pred7_", nums, sep = ""), ".txt", sep = ""), sep = ",")
  
  df =   data.frame("fit" = rep(c("CAISA","SUSIE","VARBVS","L0Learn","LASSO",
                                  "RIDGE","E-NET","BLASSO","BayesB"), each = 20),
                    "time" = c(get(paste("t", nums, sep = ""))))
  write.table(df, paste(paste("../paperresults/time7_", nums, sep = ""), ".txt", sep = ""), sep = ",")
}
```
...
for (i in 1:length(numlist)) {
  num = numlist[i]
  dat[[i]] = read.table(paste(paste("../paperresults/pred7_", num, sep = ""), ".txt", sep = ""), sep = ",")
  ...
  dat[[i]]$time = read.table(paste(paste("../paperresults/time7_", num, sep = ""), ".txt", sep = ""), sep = ",")$time
  dat[[i]]$rrmse2 = pmin(pmax(dat[[i]]$rrmse, 0),1)
  dat[[i]]$sigma = get(paste("sigma", num, sep = ""))
  dat[[i]]$p = get(paste("p", num, sep = ""))
}
saveRDS(dat, "../paperresults/experiment7.rds")
```

```{r readresults, message = FALSE}
dat = readRDS("../paperresults/experiment7.rds")
for (i in 1:9) {
...
ggsave("../paperfigures/fig71.pdf", fig1, width = 18, height = 6)
```
...
ggsave("../paperfigures/fig72.pdf", fig1, width = 18, height = 6)
```
...
ggsave("../paperfigures/fig73.pdf", fig1, width = 18, height = 6)
```
